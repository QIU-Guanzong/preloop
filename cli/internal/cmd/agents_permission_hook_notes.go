...
//     deny alone.
//   - Cursor CLI 2026.09.02 preToolUse: additional_context. beforeShellExecution
//     and beforeMCPExecution cannot carry one: additional_context is stripped
//     for every event outside sessionStart, beforeSubmitPrompt, preToolUse,
//     postToolUse and postToolUseFailure, and those two hooks read only
//     permission and user_message.
//
...
// second copy: the spool entry is removed as it is read.
...
// field that reaches the model, per the harness verification above.
func hookEventCarriesOperatorNote(source, hookEvent string) bool {
	switch source {
...
func applyOperatorNoteDelivery(
	...
	sessionID := hookEventSessionID(raw)
	if !hookEventCarriesOperatorNote(source, hookEvent) {
		if claimed != "" {
	...
	decision.OperatorNote = strings.Join(blocks, "\n\n")
	return decision
}
...
func spoolOperatorNote(source, sessionID, note string) {
	...
	entries := append(readOperatorNoteSpool(path), operatorNoteSpoolEntry{
		Note:      note,
	...
	_ = os.WriteFile(path, data, 0o600) //nolint:errcheck
}
