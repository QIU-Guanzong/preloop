...
*   **Session search:** `session_search_document` holds chunked runtime-session content (keyword `tsvector` plus an optional 1536-dimension embedding and partial HNSW index). `session_embedding_setting` is the per-account opt-in for that embedding worker.
