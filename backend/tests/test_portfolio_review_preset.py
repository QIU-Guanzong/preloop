...
class TestPresetDefinition:
    ...
    def test_declares_no_write_tools(self):
        ...
        channel, the delegation call, and get_execution used both as the
        child read and as the spend meter. No MCP server, and a prompt
        that forbids every write path including opening a pull request."""
        data = _load_preset()
        ...
        assert (
            ...
            "list, and get_execution, which reads a child you started "
            "and is the read-only meter you point at this execution to read "
            "what it has spent so far" in norm
        )
    ...
    def test_verdict_rules_are_stated(self):
        ...
        assert (
            "any failed, refused or expired child and any truncated coverage" in norm
            or "any failed, refused or expired child, any truncated coverage" in norm
        )
...
class TestSelectionQuestion:
    ...
    def test_project_cap_is_the_effective_cap(self, scenario):
        _, result, children = scenario
        declared = result["inputs_declared"]["max_projects"]
        assert result["selection"]["project_cap"] == min(declared, PROJECT_HARD_CAP)
...
class TestFanOutDeclaration:
    ...
    def test_the_child_payload_is_the_repository_project_and_depth(self):
        ...
        assert (
            "target_repo_path is the repository path selector 009 and 016 "
            "take (006 inventories the checkouts under the workspace instead)" in norm
        )
        assert (
            "depth is the budget knob 009 and 016 pass through (006 has no "
            "depth knob; its window is the per-child cost ceiling and its "
            "own timeout)" in norm
        )
    ...
    def test_a_refusal_is_an_answer_and_a_ceiling_is_declared(self):
        ...
        assert "Children of this run are depth 1" in norm
        assert "the instance depth ceiling is 2" in norm
        assert "the three lens flows carry no delegation tool" in norm
        assert "so a lens that tried to delegate again cannot" in norm
        ...
        assert (
            "children_started counts run_flow calls this run made, including "
            "calls the platform refused before a child existed" in norm
        )
        assert (
            "children_refused is a subset of children_started, not an extra "
            "bucket" in norm
        )
...
class TestFanOut:
    ...
    def test_three_projects_one_lens_start_three_children(self):
        ...
        for call in calls:
            ...
            assert arguments["payload"]["depth"] == result["selection"]["depth"]
...
class TestDepthReachesEveryLensPayload:
    ...
    def test_every_lens_payload_carries_the_resolved_depth(self, scenario):
        _, result, *_rest = scenario
        depth = result["selection"]["depth"]
        assert depth in ("quick", "standard", "deep")
        for row in result["projects"]:
            for lens in row["lenses"]:
                if lens["lens_status"] == "ran":
                    assert lens["lens_payload"] == {
                        "project_path": row["path"],
                        "depth": depth,
                    }
                else:
                    assert lens["lens_payload"] is None
    ...
    def test_with_nobody_asked_the_default_depth_stands(self):
        ...
        assert all(
            lens["lens_payload"]["depth"] == "standard"
            for row in result["projects"]
            for lens in row["lenses"]
            if lens["lens_status"] == "ran"
        )
    ...
    def test_the_preset_requires_a_depth_on_every_payload(self):
        ...
        assert (
            "projects[].lenses[].lens_payload IS THE PAYLOAD YOU ACTUALLY "
            "HANDED THE LENS" in norm
        )
...
class TestRunCeilingStopsFanOut:
    ...
    def test_the_unreached_projects_are_declared_under_coverage(self, result):
        ...
        for row in result["projects"]:
            if row["path"] in unreached:
                ...
                for lens in row["lenses"]:
                    assert lens["lens_status"] == "not_run"
                    assert lens["reason"] == "run ceiling"
                    assert lens["health"] == "unknown"
                    assert lens["lens_payload"] is None
    ...
    def test_the_ceiling_never_upgrades_a_verdict(self, result):
        ...
        for row in result["projects"]:
            if any(lens["lens_status"] == "ran" for lens in row["lenses"]):
                row["status"] = "healthy"
                for lens in row["lenses"]:
                    if lens["lens_status"] == "ran":
                        lens["verdict"] = "pass"
    ...
    def test_the_cover_declares_the_ceiling_it_hit(self):
        norm = _norm(_prompt())
        assert "THE RUN CEILING, a lens that could not run" in norm
        assert (
