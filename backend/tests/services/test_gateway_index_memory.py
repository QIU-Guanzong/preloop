...
def test_indexing_allocation_is_bounded_by_the_document_not_the_payload():
    ...
    # A copy of either payload would be ~1 MB. The bound is the document
    # itself (16k chars) plus the truncated values it is built from.
    assert peak < 2 * GatewayUsageSearchService.MAX_TEXT_CHARS
...
def test_three_concurrent_large_responses_stay_within_one_document_each():
    ...
    assert peak < CONCURRENT_RESPONSES * 4 * GatewayUsageSearchService.MAX_TEXT_CHARS
