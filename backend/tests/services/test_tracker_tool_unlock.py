...
# Folded into get_issue/update_issue in #661: no tracker can unlock a name
# the builtin registry no longer carries.
...
def test_github_never_unlocks_the_removed_triage_tool_names():
    """The triage pair folded into get_issue/update_issue (#661)."""
    unlocked = unlocked_tool_names_after_tracker(
        had_tracker=False,
        types_before=[],
        types_after=["github"],
    ...
    # Even an explicit enable row cannot resurrect a name that is gone.
    with_explicit_enable = unlocked_tool_names_after_tracker(
        had_tracker=False,
        types_before=[],
        types_after=["github"],
        enabled_by_name={name: True for name in TRIAGE_REMOVED},
    )
    assert TRIAGE_REMOVED.isdisjoint(with_explicit_enable)
    assert "update_issue" in with_explicit_enable
...
def test_supported_names_respect_default_enabled_false():
    ...
    assert TRIAGE_REMOVED.isdisjoint(names)
